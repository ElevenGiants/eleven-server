...
rsync ${DRY_RUN:+--dry-run} --compress --recursive --exclude=".git*" -e "ssh -p ${SSH_PORT}" "${src_dir}/" jenkins@${SSH_HOST}:/eleven/eleven-server.new

# run remote deployment script (moves old version out of the way, replaces it
# with new one and restarts GS service)
ssh -T -p ${SSH_PORT} jenkins@${SSH_HOST} /eleven/eleven-server.new/deploy/deploy-remote.sh
